...
Swagger UI ships with a built-in internationalization system based on a lightweight key/message look-up backed by the browser-native `Intl`-friendly string store.  No additional third-party i18n libraries are required.
...
Pass a `locale` option and a matching `i18nMessages` map when initialising Swagger UI:
...
Set `locale` to `null` (the default) and Swagger UI will read `navigator.languages[0]` / `navigator.language` and normalise it to a BCP 47 base language code (e.g. `"fr-CA"` → `"fr"`).  If no messages are registered for the detected locale, the UI falls back to English.

```js
SwaggerUIBundle({
  url: "...",
  dom_id: "#swagger-ui",
  locale: null,          // default — auto-detect
  i18nMessages: { fr: { ... } }
})
```
...
| `i18nMessages` | `object` | `{}` | Map of `{ [locale]: { [key]: translatedString } }` merged on top of the built-in English messages. |
...
- All existing config options and behaviour remain unchanged when no locale or messages are supplied.
